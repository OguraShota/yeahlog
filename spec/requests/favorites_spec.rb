...
RSpec.describe "お気に入り登録機能", type: :request do
  ...
  context "お気に入り登録処理" do
    context "ログインしている場合" do
      ...
      it "物件のお気に入り登録ができること" do
        expect {
          post "/favorites/#{property.id}/create"
        }.to change(user.favorites, :count).by(1)
      end

      it "物件のAjaxによるお気に入り登録ができること" do
        expect {
          post "/favorites/#{property.id}/create", xhr: true
        }.to change(user.favorites, :count).by(1)
      end

      it "物件のお気に入り解除ができること" do
      ...
      it "物件のAjaxによるお気に入り解除ができること" do
        user.favorite(property)
        expect {
          delete "/favorietes/#{property.id}/destroy", xhr: true
        }.to change(user.favorites, :count).by(-1)
      end
    end
    context "ログインしていない場合" do
